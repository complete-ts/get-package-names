import { checkCompiledOutputInRepo, lintCommands } from "complete-node";
...
await lintCommands(import.meta.dirname, [
  ...
  "complete-cli check --ignore action.yml,build.ts,lint.ts",

  // @template-customization-start

  // Since GitHub Actions must commit the compiled output, we must check to see if it is up-to-date.
  ["checkCompiledOutputInRepo", checkCompiledOutputInRepo()],
